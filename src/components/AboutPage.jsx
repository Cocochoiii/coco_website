...
export default function AboutPage({ go }) {
  ...
  /* ── Hooks ── */
  useParallax(leftRef,  { xFactor: -10, yFactor: -8 })
  useParallax(rightRef, { xFactor:  10, yFactor:  8 })
  useScrollReveal(scrollRef)

  /* ── Rive — about me character ── */
  useEffect(() => {
  ...
  const handleCtaEnter = useCallback((e) => {
    ...
    e.currentTarget.style.boxShadow = `0 16px 50px ${EYE.rose}50`
  }, [])

  const handleCtaLeave = useCallback((e) => {
    e.currentTarget.style.transform = ''
    e.currentTarget.style.boxShadow = `0 8px 30px ${EYE.rose}40`
  }, [])
  ...
  return (
    <div ref={scrollRef} style={{ width: '100vw', height: '100vh', background: '#fff', position: 'relative', overflowX: 'hidden', overflowY: 'auto' }}>
      ...
      <div style={{ width: '100%', height: '100vh', position: 'relative', flexShrink: 0 }}>
        ...
        {/* LEFT panel */}
        <div ref={leftRef} className="ab-left" style={{ position: 'absolute', right: '70%', top: '44%', transform: 'translateY(-50%)', width: 'clamp(280px, 28vw, 420px)', zIndex: 10, opacity: 0 }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 30, fontWeight: 700, letterSpacing: 6, textTransform: 'uppercase', color: EYE.rose, marginBottom: 20 }}>
            About Me
          </p>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(180px, 9vw, 130px)', fontWeight: 900, color: EYE.shadow, marginBottom: 24, lineHeight: 0.92, letterSpacing: -5 }}>
            Coco<br />Choi
          </h2>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(25px, 1.8vw, 30px)', lineHeight: 1.55, color: '#444', fontWeight: 400 }}>
        ...
        {/* RIGHT panel */}
        <div ref={rightRef} className="ab-right" style={{ position: 'absolute', left: '70%', top: '50%', transform: 'translateY(-50%)', width: 'clamp(280px, 28vw, 420px)', zIndex: 10, opacity: 0 }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(25px, 1.8vw, 30px)', lineHeight: 1.6, color: '#444', marginBottom: 32, fontWeight: 400 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            {ABOUT_STATS.map((st, i) => (
              ...
              >
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 44, fontWeight: 800, lineHeight: 1, color: hoveredStat === i ? '#fff' : st.color, transition: 'color 0.3s' }}>
                  <AnimatedNumber target={st.n} suffix={st.s} delay={0.8 + i * 0.2} />
                </div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: hoveredStat === i ? 'rgba(255,255,255,0.75)' : '#bbb', marginTop: 6, transition: 'color 0.3s' }}>
                  {st.label}
                </div>
              </div>
            ))}
      ...
      <div style={{ width: '100%', background: EYE.shadow, padding: 'clamp(80px, 10vw, 160px) clamp(40px, 8vw, 160px)', textAlign: 'center' }}>
        <div className="ab-reveal" style={{ maxWidth: 800, margin: '0 auto', opacity: 0 }}>
          ...
          {/* CTA button */}
          <a
            href="mailto:choi.coco@northeastern.edu"
            ...
            {CONTACT_INFO.map((c, i) => (
              <div key={i}>
                ...
                {c.href ? (
                  <a
                    href={c.href}
                    style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 15, color: EYE.cream, textDecoration: 'none', fontWeight: 500, transition: 'color 0.3s' }}
  ...
  return (
    <span
      ...
      style={{
        ...
        background: 'white', color: skill.color,
